...
# added an argument called transpose to be able to do spatial ISC with NaN TRs 05.29.2023
...
def isc(data, pairwise=False, tolerate_nans=True, transpose=False):
        ...
        for s in np.arange(n_subjects):
            ...
            if transpose:
                ...
                array_corr,intersect_mask=array_correlation(
                        data[:,:, s],
                        mean(np.delete(data, s, axis=2), axis=2),axis=1)  
            else:
                # Correlation between left-out subject and mean of others
                array_corr,intersect_mask=array_correlation(
                        data[:,:, s],
...
def array_correlation(x, y, axis=0):
    ...
    if axis == 1:
        x, y = x.T, y.T
        numerator_temp=np.nan*np.zeros(len(x))
        denominator_temp=np.nan*np.zeros(len(x))
    ...
    # Now, revert this back to the correct length if you need to  
    if axis == 1:
        numerator_temp[included_trs]=numerator
        denominator_temp[included_trs]=denominator
        diff =  numerator_temp / denominator_temp
    
    else:
        diff = numerator / denominator
        
    return diff, included_trs
