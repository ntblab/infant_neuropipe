...
subs=`cat data/EventSeg/${movie}/*_participants.csv | cut -d ',' -f1`
...
if [[ $sub != 'ppt' ]]
then
    files="${files} data/Movies/${movie}/preprocessed_standard/${alignment}/${sub}_Z.nii.gz"
fi
done


file_path=data/Movies/${movie}/preprocessed_standard/${alignment}/
mask_name=data/EventSeg/${movie}/intersect_mask_standard_firstview_all.nii.gz
