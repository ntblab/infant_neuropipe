...
usage()
{
    echo "Usage: `basename $0` -i <input 4D image> -o <output confound file> [options] --reffile <reffile>"
    ...
    echo "         -v                   verbose mode"
    echo "         --reffile <reffile>  the file to be used as the reference TR"
    echo " "
}
...
#Exit if the example_func isn't provided
if [[ $reffile == 0 ]] ; then usage; exit 1; fi
...
if [ $verbose = yes ] ; then echo "mcf = $mcf ; outfile = $outfile ; ndel = $ndel ; mask = $mask ; do_moco = $do_moco ; thresh = $abs_thresh ; use_thresh = $use_abs_thresh ; metric = $metric ; reffile = $reffile"; fi
...
if [ $do_moco = yes ] ; then
    # run mcflirt
    $FSLDIR/bin/mcflirt -in $mcf -out ${outdir}_mc/fmri_mcf -mats -plots -reffile $reffile -rmsrel -rmsabs
else 
