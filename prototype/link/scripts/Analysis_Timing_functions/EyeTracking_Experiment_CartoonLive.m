%% Summarize eye tracking data for Cartoon vs. Realistic movie experiments 
%
...
%
% Same as MM // PlayVideo // ChildPlay with some name changes
% TY 03/10/2022


function EyeData=EyeTracking_Experiment_CartoonLive(varargin)
...
is_conservative=0; % undetected TRs are treated as on-screen 

% Specify the experiment ID
Experiment = 'CartoonLive';
...
for IdxCounter=1:length(EyeData.Timecourse.(Experiment))
    ...
    MovieName = GenerateTrials.Experiment_CartoonLive.Parameters.BlockNames{Idx_Name(1)};
    ...
    % Get the time stamps
    if isfield(Data.Experiment_CartoonLive.(Block_Name).Timing.Movie_1, 'movieStart')
        
        MovieStart = Data.Experiment_CartoonLive.(Block_Name).Timing.Movie_1.movieStart.Local;
        MovieEnd = Data.Experiment_CartoonLive.(Block_Name).Timing.Movie_1.movieEnd.Local;
    else
        MovieStart = Data.Experiment_CartoonLive.(Block_Name).MovieStartTime;
        MovieEnd = MovieStart + 0.01;
    end
    ...
    % Save the figure
    saveas(gcf, sprintf('analysis/Behavioral/CartoonLive_%s.png', Block_Name));
    
    %% Convert to be a regressor
    
    % Pull out the time stamps -> from the experiment block, not the movie
    tr_timestamps_all = Data.Experiment_CartoonLive.(Block_Name).Timing.TR;
    ...
    if ~isempty(tr_timestamps_all)
        ...
        % but only do this if Semantic Story wasn't the last experiment run
        ...
        if last_bin > length(tr_timestamps_eyetracker)
            ...
            fprintf('No burnout found for CartoonLive %s\n', Block_Name);
        end
        ...
        fid = fopen(sprintf('analysis/Behavioral/CartoonLive_%s.txt', Block_Name), 'w');
        ...
        warning(sprintf('Not making a regressor for CartoonLive %s because there are no TRs for this run\n', Block_Name));
