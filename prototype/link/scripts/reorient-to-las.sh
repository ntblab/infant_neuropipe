...
for bxh_file in `ls $nifti_folder/*.bxh`; do
	...
	bxhreorient --orientation=$ORIENTATION --overwrite $temp $bxh_file
	...
	bxh2analyze --overwrite --analyzetypes --niigz --niftihdr -s $bxh_file $file_prefix

	#bxhreorient --orientation=$ORIENTATION --overwrite $bxh_file $bxh_file
